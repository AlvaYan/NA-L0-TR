...
{
  beta=betaols=glm(y ~ x-1, data=data0,family='binomial')[1]$coefficients
  ...
  for (i in 1:ite)
  {
    ...
    for (j in 1:3)
    {
      ...
      else if (j==3)
      {
        ...
        datanew=data.frame(xx,y)
        #temp=glmnet(xx,y,alpha=0,lambda=lam,standardize=FALSE,family='binomial',intercept=F)
...
DV_bi=matrix(rep(0),rep,8)#deviance for null model and MLE model and regularized model
LL_bi=matrix(rep(0),rep,7)#log likelihood for MLE and regularized model
MSE_bi=matrix(rep(0),rep,8)#mean bias for our, cp,cpr,t,tc,mle,svm
cMSE_bi=matrix(rep(0),rep,8)#mean bias for core tensor for our, cp,cpr,t,tc,mle,svm
CR_bi=matrix(rep(0),rep,9)#classification rate our, cp,cpr,t,tc,mle,svm, beta0
beta_bi_all=array(rep(0),dim=c(rep,7,p^3))#our, cp,cpr,t,tc,svm
betamle_bi_all=matrix(rep(0),rep,p^3)
