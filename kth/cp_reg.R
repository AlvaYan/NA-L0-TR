...
for (i in 1:ite)
{
  ...
  reg=cv.glmnet(xx,Y[1:n_train],alpha=0,lambda = 10^seq(1, -3, by = -1),
                standardize=FALSE,family='binomial',intercept=F)
  #lambda_best=0.00001
  lasso_model=glmnet(xx,Y[1:n_train],alpha=0,lambda=reg$lambda.min,
                     standardize=FALSE,family='binomial',intercept=F)
  Unew=array(lasso_model$beta,dim=c(P1,R))
  ...
  reg=cv.glmnet(xx,Y[1:n_train],alpha=1,lambda = 10^seq(1, -3, by = -1),
                standardize=FALSE,family='binomial',intercept=F)
  lasso_model=glmnet(xx,Y[1:n_train],alpha=1,lambda=reg$lambda.min,
                     standardize=FALSE,family='binomial',intercept=F)
  Unew=array(lasso_model$beta,dim=c(P2,R))
  Uold=U2
  Unew[which(is.na(Unew)==TRUE)]=Uold[which(is.na(Unew)==TRUE)]
  #Unew[which(Unew==0)]=Uold[which(Unew==0)]
  U2=Unew
  ...
  lasso_model=glmnet(xx,Y[1:n_train],alpha=0,lambda=reg$lambda.min,
                     standardize=FALSE,family='binomial',intercept=F)
  Unew=array(lasso_model$beta,dim=c(P3,R))
  ...
  if (i>1)
  {
    ...
    #if ((sum(abs(Bold-B))+sum(abs(Rold-RR)))/(length(RR)+length(B))<stop)
