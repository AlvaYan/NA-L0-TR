...
for (i in 1:ite)
{
  ...
  reg=cv.glmnet(xx,Y[1:n_train],alpha=0.8,lambda = 10^seq(0, -2, by = -1),
             standardize=FALSE,family='binomial',intercept=F)
  #           offset = Z[1:n_train,]%*%(R))
  #lambda_best=0.00001
  lasso_model=glmnet(xx,Y[1:n_train],alpha=0.8,lambda=reg$lambda.min,
                     standardize=FALSE,family='binomial',intercept=F)
  #                   offset = Z[1:n_train,]%*%(R))
  Gnew=array(lasso_model$beta,dim=c(p1,p2,p3))
  Gold=G
