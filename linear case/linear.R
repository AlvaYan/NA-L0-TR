...
tuc=function (U,G,x,y)#U is array 3*p*p,G is tensor p*p*p. U, G are initial value and x, y are data
{
  ite=5000
...
cp_reg = function(U, x, y) { # U is array 3*R*p initial value
  ite = 5000
  ll = matrix(0, ite, 1)
  betamat = NULL
  beta_best = 0.1
  ...
  for (i in 1:ite) {
    # Update each U (eigenvector) according to Tucker Tensor Regression
    for (j in 1:3) {
      xx = matrix(0, n, p * R) # Data for GLM regression to update Uj
      if (j == 1) {
        # Update U[1,,] using Tucker decomposition
        L = list('mat1' = matrix(U[3,,], p, R), 'mat2' = matrix(U[2,,], p, R))
        kp = khatri_rao_list(L)
        for (k in 1:n) {
          Xn = k_unfold(as.tensor(array(x[k,], dim = c(p, p, p))), 1)@data
          xx[k, ] = matrix(Xn %*% kp, 1, R * p)
        }
        lambdas <- 10^seq(-1, -5, by = -0.1)
        lasso_reg <- cv.glmnet(xx, y, alpha = 0, lambda = lambdas, standardize = FALSE, nfolds = 5, intercept = FALSE)
        lambda_best <- lasso_reg$lambda.min
        ...
        # Update U[2,,] using Tucker decomposition
        L = list('mat1' = matrix(U[3,,], p, R), 'mat2' = matrix(U[1,,], p, R))
        kp = khatri_rao_list(L)
        for (k in 1:n) {
          Xn = k_unfold(as.tensor(array(x[k,], dim = c(p, p, p))), 2)@data
          xx[k, ] = matrix(Xn %*% kp, 1, R * p)
        }
        lambdas <- 10^seq(-1, -5, by = -0.1)
        lasso_reg <- cv.glmnet(xx, y, alpha = 0, lambda = lambdas, standardize = FALSE, nfolds = 5, intercept = FALSE)
        ...
        # Update U[3,,] using Tucker decomposition
        L = list('mat1' = matrix(U[2,,], p, R), 'mat2' = matrix(U[1,,], p, R))
        kp = khatri_rao_list(L)
        for (k in 1:n) {
          Xn = k_unfold(as.tensor(array(x[k,], dim = c(p, p, p))), 3)@data
          xx[k, ] = matrix(Xn %*% kp, 1, R * p)
        }
        lambdas <- 10^seq(-1, -5, by = -0.1)
        lasso_reg <- cv.glmnet(xx, y, alpha = 0, lambda = lambdas, standardize = FALSE, nfolds = 5, intercept = FALSE)
    ...
    if (i > 1 && sum(abs(betamat[i, ] - betamat[i - 1, ])) < stop) {
      beta = kp %*% matrix(rep(1), R, 1)
      return(list(beta, 1, ll[i, 1]))
    }
  }
  
  return(list(beta, 0, ll[ite, 1]))
}
cp_reg = function(U, x, y) { # U is array 3*R*p initial value
  ite = 5000
  ll = matrix(0, ite, 1)
...
MSE=matrix(rep(0),rep,7)#mean bias
PMAE=matrix(rep(0),rep,8)#prediction MSE
beta_all=array(rep(0),dim=c(rep,7,p^3))
#betaols_all=matrix(rep(0),rep,p^3)
cMSE=matrix(rep(0),rep,7)#mean bias for core tensor
LL=matrix(rep(0),rep,7)#llh function
cvg=matrix(rep(0),rep,6)#1 is converge,2 is not cvg
...
#save(betaols_all,file="betaols_all.Rdata")
save(cvg,file="cvg.Rdata")
save(LL,file="LL.Rdata")
save(time,file="time_linear.Rdata")

#beta1=colMeans(beta_all[,1,])
#beta2=colMeans(beta_all[,2,])
#beta3=colMeans(beta_all[,3,])
#beta4=colMeans(beta_all[,4,])
#beta4=colMeans(beta_all[,5,])
#betaols=colMeans(betaols_all)
round((aRMSE=colMeans(RMSE)),4)
round((aPMSE=colMeans(PMSE)),4)
round((acMSE=colMeans(cMSE)),4)
